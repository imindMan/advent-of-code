...
let perfect_symmetry (arr : string array) list_holding const_index: int * int =
  let rec check pre_change const_index pass_it =
    let i = if pass_it = false then if const_index = 0 then const_index else pre_change else pre_change in
    let j = if pass_it = false then if const_index <> 0 then const_index else pre_change else const_index in
