...
let perfect_symmetry (arr : string array) prefix : int * int =
  ...
  let rec check i j prefix =
    if i >= j then (i + prefix, j + prefix)
      (* Base case: the entire array has been checked *)
    else if arr.(i) = arr.(j) then
      check (i + 1) (j - 1) prefix (* Move towards the center *)
    else (-1, -1)
...
let symmetry (arr : string array) =
  ...
  if res_head = -1 && res_tail = -1 then (-1, -1)
    ...
    let tail =
      perfect_symmetry
        (Array.sub arr res_tail (Array.length arr - res_tail))
        res_tail
    in
    if head <> (-1, -1) then tail else head
  else if res_tail <> -1 then
    perfect_symmetry
      (Array.sub arr res_tail (Array.length arr - res_tail))
      res_tail
