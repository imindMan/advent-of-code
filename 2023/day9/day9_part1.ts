...
lineReader.on('line', (line) => {
    ...
    sum += tracing_history[0][tracing_history[0].length - 1];
...
function extrapolate(tracing_history: number[][]) {
    if (tracing_history.length == 1) {
        ...
        tracing_history[tracing_history.length - 1].push(0)
        let next_array = tracing_history[tracing_history.length - 2];
        let end_array = tracing_history[tracing_history.length - 1];
        tracing_history[tracing_history.length - 2].push(
            end_array[end_array.length - 1] + next_array[next_array.length - 1]
        )
